...
export default function Home() {
  ...
  }, [pagina, catActiva, loadingMas, hayMas]);
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <style>{`
        ...
        @media (min-width: 480px) {
          .hk-grid { grid-template-columns: repeat(3, 1fr); gap: 12px; padding: 16px 20px; }
        }
        @media (min-width: 768px) {
          .hk-grid { grid-template-columns: repeat(4, 1fr); padding: 20px 28px; }
        }
        @media (min-width: 1280px) {
          .hk-grid { grid-template-columns: repeat(6, 1fr); }
        }
        .hk-strip {
          background: #111;
          color: #fff;
        ...
        @media (min-width: 768px) {
          .hk-strip { padding: 36px 28px; flex-direction: row; align-items: center; justify-content: space-between; gap: 0; }
      ...
      <div className="hk-hero" style={{ background: b.bg, transition: 'background 0.5s ease' }}>
        ...
        <div className="hk-hero-img" style={{
          background: b.imgBg, alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ position: 'absolute', bottom: 18, right: 18, background: 'rgba(255,255,255,0.92)', padding: '9px 16px' }}>
            <div style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#111', fontWeight: 700 }}>
              Nueva temporada
            </div>
      ...
      {!loading && hayMas && productos.length > 0 && (
        <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 16px 32px' }}>
          <button
            ...
            disabled={loadingMas}
            style={{
              background: loadingMas ? '#f0ede8' : '#111',
              color: loadingMas ? '#999' : '#fff',
              border: 'none',
              padding: '13px 40px',
              fontSize: 11,
              ...
              fontWeight: 700,
              cursor: loadingMas ? 'not-allowed' : 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: 8,
              transition: 'all 0.2s',
          ...
          >
            {loadingMas
              ? <><span style={{ display: 'inline-flex', animation: 'spin 1s linear infinite' }}>⟳</span> Cargando...</>
              : 'Cargar más'
            }
          </button>
        </div>
